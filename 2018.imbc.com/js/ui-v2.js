$(window).on({
	load : function() {
		...
		if($('.main').length) {
			...
			$('.wrap-schedule .btn-show').click(function(){
				$('.wrap-schedule .today-schedule').slideDown();
				$('.wrap-schedule .today-schedule').css('opacity','1');
				$('.wrap-now').hide();
			});
			$('.today-schedule .btn-up').click(function(){
				$('.today-schedule').hide();
				$('.today-schedule').css('opacity','0');
				$('.wrap-now').show();
			})
		};
		if($('.sub-vod').length) {
...
function vodMenuSlide() {
  //vod sort
		    var w = window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;
			var scroll;
				var list = document.getElementById("vodMenu");
				if (list) {
					scroll = new IScroll('#vodMenu', { scrollX: true, scrollY: false, mouseWheel: true, click:true });
					list.addEventListener('touchmove', function (e) { e.preventDefault(); }, false);
				}
}
function itemSlide(){
